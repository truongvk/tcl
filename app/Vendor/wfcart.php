...
// constructor function
    ...
    function get_contents() { // gets cart contents
        ...
        foreach ($this->items as $tmp_item) {
            ...
            $item['id']     = $tmp_item;
            $item['qty']    = (isset($this->itemqtys[$tmp_item])) ? $this->itemqtys[$tmp_item] : null;
            $item['price']  = (isset($this->itemprices[$tmp_item])) ? $this->itemprices[$tmp_item] : null;
            $item['info']   = (isset($this->iteminfo[$tmp_item])) ? $this->iteminfo[$tmp_item] : null;
            $item['extra']  = (isset($this->itemExtra[$tmp_item])) ? $this->itemExtra[$tmp_item] : null;
            $item['subtotal'] = intval($item['qty']) * $item['price'];
            $items[] = $item;
        }
        return $items;
    }
...
// end of get_contents
    ...
    function add_item($itemid, $qty=1, $price = FALSE, $info = FALSE, $extra=null) { // adds an item to cart
        if (!$price) {
        ...
        if (isset($this->itemqtys[$itemid]) && $this->itemqtys[$itemid] > 0) { // the item is already in the cart..
